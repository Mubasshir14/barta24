...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout, lang }) => {
  ...
  const handleAutoTranslate = async () => {
    if (!currentEdit.title?.bn || !currentEdit.content?.bn) {
      alert('Please enter Bengali content first to auto-translate.');
      return;
    }
        ...
        {isEditing ? (
          <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 border-2 border-black dark:border-gray-700 p-8 space-y-10 shadow-[10px_10px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div 
                onClick={() => fileInputRef.current?.click()}
                className="md:col-span-1 aspect-video bg-gray-50 dark:bg-gray-900 border-2 border-dashed border-gray-300 dark:border-gray-600 flex flex-col items-center justify-center cursor-pointer overflow-hidden relative"
              >
                {currentEdit.image ? (
                  <img src={currentEdit.image} className="w-full h-full object-cover" />
                ) : (
                  <div className="text-center opacity-30">
                    <Upload className="mx-auto mb-2" />
                    <p className="font-black text-[10px]">UPLOAD IMAGE</p>
                  </div>
                )}
                <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFile} />
              </div>
              ...
              <div className="md:col-span-2 grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col justify-center gap-2">
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-xs">
                    <input type="checkbox" checked={currentEdit.isBreaking} onChange={e => setCurrentEdit({...currentEdit, isBreaking: e.target.checked})} className="w-4 h-4 accent-red-700"/> Breaking
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer font-bold text-xs">
                    <input type="checkbox" checked={currentEdit.isFeatured} onChange={e => setCurrentEdit({...currentEdit, isFeatured: e.target.checked})} className="w-4 h-4 accent-red-700"/> Featured
                  </label>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8">
              <div className="flex justify-center">
                <button 
                  ...
                  disabled={isTranslating}
                  onClick={handleAutoTranslate}
                  className="bg-black text-white px-6 py-3 font-black uppercase text-[10px] tracking-widest flex items-center gap-3 hover:bg-red-700 transition-colors disabled:opacity-50"
                >
                  {isTranslating ? <Loader2 size={16} className="animate-spin"/> : <Sparkles size={16}/>}
                  AI Magic: Translate Bengali to English
                </button>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                {/* Bengali Section */}
                <div className="space-y-6 bg-gray-50/50 dark:bg-gray-900/20 p-6 border-l-4 border-red-700">
                  <h3 className="text-sm font-black uppercase text-red-700">Bengali Version (বাংলা)</h3>
                  
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-gray-400">Headline</label>
                    <input 
                      placeholder="শিরোনাম লিখুন..."
                      className="w-full p-4 border-2 border-black dark:border-gray-700 bg-white dark:bg-gray-800 font-bold text-lg outline-none focus:border-red-700"
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-gray-400">Excerpt</label>
                    <textarea 
                      placeholder="সংক্ষিপ্ত সারসংক্ষেপ..."
                      rows={2}
                      className="w-full p-4 border-2 border-black dark:border-gray-700 bg-white dark:bg-gray-800 font-bold outline-none focus:border-red-700"
                  ...
                  <div className="space-y-1">
                    ...
                    <textarea 
                      ...
                      rows={12}
                      className="w-full p-4 border-2 border-black dark:border-gray-700 bg-white dark:bg-gray-800 font-medium outline-none focus:border-red-700"
                ...
                {/* English Section */}
                <div className="space-y-6 bg-gray-50/50 dark:bg-gray-900/20 p-6 border-l-4 border-blue-700">
                  <h3 className="text-sm font-black uppercase text-blue-700">English Version</h3>
                  
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-gray-400">Headline (English)</label>
                    <input 
                      placeholder="Enter headline..."
                      className="w-full p-4 border-2 border-black dark:border-gray-700 bg-white dark:bg-gray-800 font-bold text-lg outline-none focus:border-blue-700"
                      value={currentEdit.title?.en || ''}
                      onChange={e => setCurrentEdit({...currentEdit, title: {...currentEdit.title, en: e.target.value} as any})}
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-gray-400">Excerpt (English)</label>
                    <textarea 
                      placeholder="Short summary..."
                      rows={2}
                      className="w-full p-4 border-2 border-black dark:border-gray-700 bg-white dark:bg-gray-800 font-bold outline-none focus:border-blue-700"
                      value={currentEdit.excerpt?.en || ''}
                      onChange={e => setCurrentEdit({...currentEdit, excerpt: {...currentEdit.excerpt, en: e.target.value} as any})}
                    />
                  </div>

                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-gray-400">Content (English)</label>
                    <textarea 
                      placeholder="Full article..."
                      rows={12}
                      className="w-full p-4 border-2 border-black dark:border-gray-700 bg-white dark:bg-gray-800 font-medium outline-none focus:border-blue-700"
                      value={currentEdit.content?.en || ''}
                      onChange={e => setCurrentEdit({...currentEdit, content: {...currentEdit.content, en: e.target.value} as any})}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-gray-800 border-2 border-black dark:border-gray-700 overflow-hidden shadow-[5px_5px_0px_0px_rgba(0,0,0,1)]">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700 font-bold text-sm">
                  {articles.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="p-20 text-center opacity-30 italic">No articles found</td>
                    </tr>
                  ) : articles.map(a => (
                    <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                      <td className="p-4 flex gap-4 items-center">
                        <img src={a.image} className="w-12 h-8 object-cover border border-black/10 shrink-0" />
                        <span className="line-clamp-1">{a.title.bn}</span>
                      </td>
                      <td className="p-4"><span className="text-[10px] bg-black text-white px-2 py-0.5 uppercase">{a.category}</span></td>
