...
import { Clock, Eye, Facebook, Twitter, Link, ArrowLeft, MessageSquare } from 'lucide-react';
...
const NewsDetail: React.FC<NewsDetailProps> = ({ article, lang, onBack, onSelectRelated }) => {
  ...
  const loadRelated = async () => {
    try {
      const all = await BartaAPI.getLatestNews(15);
      const filtered = all
          ...
          <aside className="lg:col-span-4 space-y-8">
            ...
            <div className="sticky top-24 space-y-8">
              <div className="bg-gray-50 dark:bg-gray-800/30 p-6 rounded-xl border border-gray-100 dark:border-gray-700">
                <h3 className="font-bold border-b-2 border-red-700 pb-2 mb-6 uppercase text-sm tracking-widest">
                  {lang === 'bn' ? 'আরও পড়ুন' : 'Read More'}
                </h3>
                <div className="space-y-6">
                  {relatedArticles.length === 0 ? (
                    <div className="space-y-4">
                      {[1,2,3].map(i => <div key={i} className="h-20 bg-gray-200 dark:bg-gray-700 animate-pulse rounded" />)}
                    </div>
                    ...
                    >
                      <img src={rel.image} className="w-20 h-20 object-cover rounded shadow-sm border border-black/10 shrink-0" />
                      <div className="flex-1 overflow-hidden">
                        <h4 className="text-sm font-bold leading-snug group-hover:text-red-700 transition-colors line-clamp-2">
                          {rel.title[lang]}
                        </h4>
                        <span className="text-[10px] opacity-60 uppercase font-bold mt-1 block">
